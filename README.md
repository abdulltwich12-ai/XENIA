...
Sito che consiglia i migliori prodotti elettronici in base a una richiesta in linguaggio naturale: cerca i prodotti reali su Google Shopping (via SerpApi), li fa confrontare e classificare da un'AI gratuita (Google Gemini), e mostra una lista curata con foto, prezzo e link d'acquisto diretto verso il negozio.
...
2. Copia `.env.local.example` in `.env.local` e compila le chiavi:
   ...
   - `GEMINI_API_KEY` — gratuita su [aistudio.google.com/apikey](https://aistudio.google.com/apikey)
...
- `lib/ai.ts` — client Google Gemini, prompt di confronto/ranking, parsing dell'output JSON
...
Push su GitHub e importa il repo su [Vercel](https://vercel.com/new): ricordati di impostare le stesse variabili d'ambiente (`GEMINI_API_KEY`, `SERPAPI_KEY`) nelle impostazioni del progetto Vercel.
